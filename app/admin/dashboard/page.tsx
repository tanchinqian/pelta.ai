    ...
    <div className="flex-1 p-4 space-y-3 max-w-[1400px] mx-auto w-full">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-mono text-text-tertiary">/ overview</span>
        </div>
      ...
      >
        ...
        <div className="col-span-3 panel p-3">
          <span className="text-sm font-semibold text-text-secondary uppercase tracking-wider">Detection Method</span>
            ...
            {detectionSplit.map((d) => {
              ...
              return (
                <div key={d.name} className="flex-1 text-center">
                  ...
                  <div className="text-xs text-text-tertiary mt-0.5">{d.name}</div>
                  <div className="text-xs font-mono text-text-secondary">{d.value} scans</div>
                </div>
              );
            })}
          </div>
        ...
        <div className="col-span-5 panel p-3">
          <span className="text-sm font-semibold text-text-secondary uppercase tracking-wider">NIST RMF Coverage</span>
          <span className="text-xs font-mono text-text-tertiary ml-2">approved tools</span>
        ...
        <div className="col-span-4 panel p-3">
          <span className="text-sm font-semibold text-text-secondary uppercase tracking-wider">Verdict Trend</span>
          <span className="text-xs font-mono text-text-tertiary ml-2">last 7 days</span>
          <div style={{ width: '100%', height: 140 }}>
            <ResponsiveContainer width="100%" height="100%">
                  ...
                  {sortedTools.map((t, i) => (
                    <tr key={t.id} className={`border-b border-border/40 hover:bg-surface-hover/50 transition-colors ${i % 2 === 1 ? 'bg-surface-hover/20' : ''}`}>
                      ...
                      <td className="py-1.5 pr-3">
                        {t.riskTier
                          ? <span style={{ color: riskColor(t.riskTier) }} className="text-xs font-bold font-mono uppercase">{t.riskTier}</span>
                      ...
                      <td className="py-1.5 pl-3 text-xs font-mono text-text-tertiary hidden lg:table-cell">{t.nistFunctions.join(', ') || '—'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            <tbody>
              {visibleLogs.map((l, i) => (
                <tr key={l.id} className={`border-b border-border/40 hover:bg-surface-hover/50 transition-colors ${i % 2 === 1 ? 'bg-surface-hover/20' : ''}`}>
                  <td className="py-1.5 pr-3 text-xs font-mono text-text-tertiary whitespace-nowrap">{new Date(l.timestamp).toLocaleTimeString()}</td>
                  <td className="py-1.5 pr-3">
                  ...
                  <td className="py-1.5 pr-3 hidden sm:table-cell">
                    <span style={{ color: riskColor(l.riskLevel === 'none' ? 'Low' : l.riskLevel) }} className="text-xs font-mono uppercase">{l.riskLevel}</span>
                  </td>
                  <td className="py-1.5 pr-3 hidden md:table-cell">
                    <span className={`text-xs font-mono ${l.source === 'extension' ? 'text-accent' : 'text-text-tertiary'}`}>{l.source ?? 'manual'}</span>
                  </td>
                  <td className="py-1.5 pr-3 hidden md:table-cell text-xs font-mono text-text-tertiary">{(l as any).tool ?? '—'}</td>
                  <td className="py-1.5 text-xs font-mono text-text-tertiary truncate max-w-[180px]">{l.promptSnippet}</td>
                </tr>
              ))}
            </tbody>
          </table>
  ...
  return (
    <div className="panel p-3">
      <div className="flex items-center justify-between mb-1">
        <span className="text-sm font-semibold text-text-secondary uppercase tracking-wider">{title}</span>
        {subtitle && <span className="text-xs font-mono text-text-tertiary">{subtitle}</span>}
      </div>
      {children}
    </div>
  );
