...
function AppealDetailModal({
  ...
  return (
    ...
    >
      <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 shadow-sm rounded-lg w-full max-w-lg animate-slide-in" style={{ maxHeight: '90vh', overflowY: 'auto' }}>
        <div className="flex items-center justify-between px-4 py-3 border-b border-zinc-200 dark:border-zinc-800">
          <div className="flex items-center gap-2">
            <ShieldAlert size={13} className="text-accent" />
            <span className="text-base font-semibold text-zinc-900 dark:text-zinc-100">Redress Appeal</span>
          </div>
          <button onClick={onClose} className="text-zinc-500 dark:text-zinc-400 hover:text-zinc-700 dark:hover:text-zinc-300 transition-colors cursor-pointer text-sm font-mono px-2 py-1 rounded hover:bg-zinc-100 dark:hover:bg-zinc-800">ESC</button>
        </div>
        <div className="p-4 space-y-4">
          <div className="flex items-start justify-between gap-3">
            <div>
              <p className="text-base font-semibold text-zinc-900 dark:text-zinc-100">{req.employeeName}</p>
              <p className="text-sm font-mono text-zinc-500 dark:text-zinc-400 mt-0.5">ID: {req.id.slice(0, 16)}…</p>
            </div>
          ...
          <div>
            <p className="text-sm font-mono uppercase tracking-widest text-zinc-500 dark:text-zinc-400 mb-1.5">Requested Data Categories</p>
          ...
          <div className="grid grid-cols-2 gap-2">
            <div className="bg-zinc-50 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-lg p-3">
              <p className="text-sm font-mono uppercase tracking-widest text-zinc-500 dark:text-zinc-400">Submitted</p>
              <p className="text-base font-mono text-zinc-600 dark:text-zinc-300 mt-0.5">{fmt(req.requestedAt)}</p>
            </div>
            <div className="bg-zinc-50 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-lg p-3">
              <p className="text-sm font-mono uppercase tracking-widest text-zinc-500 dark:text-zinc-400">Decided</p>
              <p className="text-base font-mono text-zinc-600 dark:text-zinc-300 mt-0.5">{req.decidedAt ? fmt(req.decidedAt) : '—'}</p>
            </div>
          </div>
          {req.reviewerName && (
            <div>
              <p className="text-sm font-mono uppercase tracking-widest text-zinc-500 dark:text-zinc-400 mb-1">Reviewed by</p>
              <p className="text-sm font-mono text-zinc-600 dark:text-zinc-300">{req.reviewerName}</p>
            </div>
          )}
          {req.adminComment && (
            <div>
              <p className="text-sm font-mono uppercase tracking-widest text-zinc-500 dark:text-zinc-400 mb-1">Admin Comment</p>
              <p className="text-base text-zinc-600 dark:text-zinc-300 leading-relaxed bg-zinc-50 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-lg p-3">{req.adminComment}</p>
            </div>
          )}
          {isPending && (
            <div>
              {!isRejecting ? (
                <div className="flex items-center gap-2">
                  <button id={`approve-modal-${req.id}`} onClick={() => onApprove(req.id)}
                    className="flex items-center gap-1.5 text-base font-semibold text-risk-low bg-risk-low/10 hover:bg-risk-low/20 border border-risk-low/30 rounded-lg px-3 py-1.5 transition-colors cursor-pointer">
                ...
                <div className="space-y-2">
                  <textarea
                    className="w-full bg-zinc-50 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-lg px-3 py-2 text-base text-zinc-900 dark:text-zinc-100 placeholder-zinc-500 dark:placeholder-zinc-400 focus:outline-none focus:border-accent transition-colors resize-none"
...
function ToolDetailModal({
  ...
  return (
    <div
      ...
      onClick={(e) => { if (e.target === e.currentTarget) onClose(); }}
    >
      <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 shadow-sm rounded-lg w-full max-w-md animate-slide-in">
        <div className="flex items-center justify-between px-4 py-3 border-b border-zinc-200 dark:border-zinc-800">
          <div className="flex items-center gap-2">
            <Briefcase size={13} className="text-accent" />
            <span className="text-base font-semibold text-zinc-900 dark:text-zinc-100">Tool Request</span>
          </div>
          <button onClick={onClose} className="text-zinc-500 dark:text-zinc-400 hover:text-zinc-700 dark:hover:text-zinc-300 transition-colors cursor-pointer text-sm font-mono px-2 py-1 rounded hover:bg-zinc-100 dark:hover:bg-zinc-800">ESC</button>
        </div>
        <div className="p-4 space-y-4">
          <div className="flex items-start justify-between gap-3">
            <div>
              <p className="text-base font-semibold text-zinc-900 dark:text-zinc-100">{req.employeeName}</p>
              <p className="text-sm font-mono text-zinc-500 dark:text-zinc-400 mt-0.5">{req.department}</p>
            </div>
            <StatusBadge status={req.status} />
          </div>
          <div className="bg-zinc-50 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-lg p-3">
            <p className="text-sm font-mono uppercase tracking-widest text-zinc-500 dark:text-zinc-400 mb-1">Requested Tool</p>
            <p className="text-base font-medium text-zinc-900 dark:text-zinc-100">{req.toolRequested}</p>
          </div>
          <div className="grid grid-cols-2 gap-2">
            <div className="bg-zinc-50 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-lg p-3">
              <p className="text-sm font-mono uppercase tracking-widest text-zinc-500 dark:text-zinc-400">Submitted</p>
              ...
              {!isDenying ? (
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button onClick={onCancelDeny} className="text-base text-zinc-500 dark:text-zinc-400 hover:text-zinc-700 dark:hover:text-zinc-300 transition-colors cursor-pointer">Cancel</button>
                  </div>
                </div>
              )}
            </div>
...
function StatMini({ label, value, color }: { label: string; value: number; color: string }) {
  return (
    <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 shadow-sm rounded-lg px-3 py-2">
      <p className="text-sm font-mono uppercase tracking-widest text-zinc-500 dark:text-zinc-400 font-semibold">{label}</p>
  ...
  if (loading) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center gap-4">
        ...
        <span className="text-sm text-text-tertiary font-mono">Loading requests...</span>
      </div>
    );
  }
    ...
    <>
      <div className="flex-1 p-4 max-w-5xl mx-auto w-full space-y-4">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between pb-4 border-b border-zinc-200 dark:border-zinc-800 gap-3">
          ...
          {totalPending > 0 && (
            <span className="text-sm font-bold font-mono px-2 py-1 rounded self-start sm:self-auto"
        ...
        {/* Tabs */}
        <div className="flex items-center border-b border-zinc-200 dark:border-zinc-800 gap-0">
          <TabButton
        ...
        {activeTab === 'appeals' && (
          ...
          >
            {/* Filter tabs + Search */}
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 shadow-sm rounded-lg p-2.5">
              <div className="flex items-center gap-1.5 text-base font-medium flex-wrap">
                {(['all', 'pending', 'approved', 'rejected'] as const).map((f) => {
                  ...
                  return (
                    <button key={f} id={`appeal-filter-${f}`} onClick={() => setAppealFilter(f)}
                      className={`flex items-center gap-1.5 px-2.5 py-1 rounded-lg capitalize transition-colors cursor-pointer ${
                        appealFilter === f ? 'bg-zinc-100 dark:bg-zinc-800 text-zinc-900 dark:text-zinc-100 border border-zinc-200 dark:border-zinc-700' : 'text-zinc-500 dark:text-zinc-400 border border-transparent hover:text-zinc-700 dark:hover:text-zinc-300 hover:bg-zinc-50 dark:hover:bg-zinc-800/50'
                      }`}>
                      {f}
                      <span className={`text-sm font-mono px-1 py-0.5 rounded leading-none ${appealFilter === f ? 'bg-zinc-50 dark:bg-zinc-950 text-zinc-600 dark:text-zinc-300' : 'bg-zinc-50 dark:bg-zinc-800 text-zinc-500 dark:text-zinc-400'}`}>
                        {count}
                      </span>
                    </button>
                  );
              ...
              <input
                ...
                className="bg-zinc-50 dark:bg-zinc-950 text-zinc-900 dark:text-zinc-100 border border-zinc-200 dark:border-zinc-800 rounded-lg px-3 py-1.5 text-base placeholder-zinc-500 dark:placeholder-zinc-400 focus:outline-none focus:border-accent w-full sm:w-48 transition-colors"
              />
            </div>
            ...
            <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 shadow-sm rounded-lg overflow-hidden">
              <button id="toggle-audit-log" onClick={() => setShowAudit((v) => !v)}
                className="w-full flex items-center justify-between px-4 py-3 text-left hover:bg-zinc-50 dark:hover:bg-white/[0.02] transition-colors cursor-pointer">
                <div className="flex items-center gap-2">
                  <FileText size={13} className="text-zinc-500 dark:text-zinc-400" />
                  <span className="text-base font-mono uppercase tracking-widest text-zinc-700 dark:text-zinc-200 font-semibold">Audit Trail</span>
                  <span className="text-sm font-mono text-zinc-500 dark:text-zinc-400">({auditLog.length} entries)</span>
                </div>
                {showAudit ? <ChevronUp size={13} className="text-zinc-500 dark:text-zinc-400" /> : <ChevronDown size={13} className="text-zinc-500 dark:text-zinc-400" />}
              </button>
              {showAudit && (
                <div className="border-t border-zinc-200 dark:border-zinc-800">
                  {auditLog.length === 0 ? (
                    <p className="text-base text-zinc-500 dark:text-zinc-400 text-center py-4">No audit entries yet.</p>
                  ) : (
                      ...
                      <table className="w-full text-sm">
                        <thead className="sticky top-0 bg-zinc-100 dark:bg-zinc-800">
                          <tr className="text-left text-zinc-700 dark:text-zinc-200 border-b border-zinc-200 dark:border-zinc-800">
                            ...
                            <th className="py-2 pr-3 font-semibold hidden sm:table-cell">Reviewer</th>
                            <th className="py-2 pr-3 font-semibold hidden md:table-cell">Categories</th>
                            <th className="py-2 pr-3 font-semibold hidden md:table-cell">Risk</th>
                            <th className="py-2 font-semibold hidden lg:table-cell">Comment</th>
                          </tr>
                        </thead>
                        <tbody className="divide-y divide-zinc-200 dark:divide-zinc-800/80">
                          {auditLog.map((entry, i) => (
                            <tr key={entry.id} className={`${i % 2 === 1 ? 'bg-zinc-50/50 dark:bg-white/[0.02]' : ''}`}>
                ...
                {(['all', 'pending', 'approved', 'denied'] as const).map((f) => {
                  ...
                  return (
                    <button key={f} id={`tool-filter-${f}`} onClick={() => setToolFilter(f)}
                      className={`flex items-center gap-1.5 px-2.5 py-1 rounded-lg capitalize transition-colors cursor-pointer ${
                        toolFilter === f ? 'bg-zinc-100 dark:bg-zinc-800 text-zinc-900 dark:text-zinc-100 border border-zinc-200 dark:border-zinc-700' : 'text-zinc-500 dark:text-zinc-400 border border-transparent hover:text-zinc-700 dark:hover:text-zinc-300 hover:bg-zinc-50 dark:hover:bg-zinc-800/50'
                      }`}>
                      {f}
                      <span className={`text-sm font-mono px-1 py-0.5 rounded leading-none ${toolFilter === f ? 'bg-zinc-50 dark:bg-zinc-950 text-zinc-600 dark:text-zinc-300' : 'bg-zinc-50 dark:bg-zinc-800 text-zinc-500 dark:text-zinc-400'}`}>
                        {count}
                      </span>
                    </button>
                  );
    ...
    <button
      ...
      className={`flex items-center gap-1.5 px-4 py-2.5 text-sm font-medium border-b-2 transition-colors cursor-pointer ${
        active
          ...
          : 'border-transparent text-zinc-500 dark:text-zinc-400 hover:text-zinc-700 dark:hover:text-zinc-300 hover:border-zinc-200 dark:hover:border-zinc-700'
      }`}
    >
      {icon}
      {label}
      {badge > 0 && (
        <span className="text-sm font-bold font-mono px-1 py-0.5 rounded leading-none"
